"""Deterministic contracts for Device Configuration discovery."""
...
class TestAPIDiscoveryFeature:
    ...
    def _verify_discovery_result(self, result):
        ...
        assert isinstance(first_api_versions, dict), (
            "API versions should be a dictionary"
        )
    ...
    def test_url_properties(self, client):
        ...
        assert api.rest_api_url == "No REST API available", (
            "Missing REST API URL should return empty string"
        )
        assert api.rest_ui_url == "No REST UI available", (
            "Missing Swagger UI URL should return empty string"
        )

    @pytest.mark.unit
    def test_api_creation_requires_documented_fields(self):
        """Reject incomplete DCA entries instead of inventing defaults."""
        with pytest.raises(FeatureError):
            DiscoveredAPI.from_discovery_data("test-api", "v1", {})
