...
class ThreadedHTTPServer(ThreadingMixIn, HTTPServer):
    ...
    def handle_error(self, request, client_address):
        ...
        if issubclass(exc_type, (ConnectionResetError, BrokenPipeError, ConnectionAbortedError)):
            print(f"Connection error with {client_address}: {exc_value}")
        else:
...
class MockDeviceHandler(BaseHTTPRequestHandler):
    ...
    def _handle_request(self, method):
        ...
        print(f"Thread {thread_id}: Received cookie header: {cookie_header}")
        session_token = None
        ...
        with MockDeviceHandler.session_lock:
                    ...
                    for part in session_parts:
                        if part.startswith('session='):
                            ...
                            if token in MockDeviceHandler.session_tokens:
                                session_token = token
                                print(f"Thread {thread_id}: Found valid session token: {token}")
                                break
                            else:
                                print(f"Thread {thread_id}: Found invalid session token: {token}")
                except (IndexError, ValueError) as e:
                    # If parsing fails, we'll create a new token
                    print(f"Thread {thread_id}: Error parsing cookie: {e}")
                    pass
            ...
            if not session_token:
                if MockDeviceHandler.use_fixed_session_token:
                    ...
                    print(f"Thread {thread_id}: Creating new fixed session token: {session_token}")
                else:
                    # Generate sequential tokens for session tests
                    session_token = f"mock-session-{len(MockDeviceHandler.session_tokens)}"
                    print(f"Thread {thread_id}: Creating new sequential session token: {session_token}")
        ...
        if route_key in self.routes:
            ...
            print(f"[MockDeviceHandler] No route found for '{route_key}'. Available routes: {list(self.routes.keys())}")
