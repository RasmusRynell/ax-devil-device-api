...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "skip_health_check: mark test to skip automatic health checks"
    )
...
def client(protocol):
    ...
    if protocol == "http":
        config = DeviceConfig.http(
            host=device_ip, username=device_user, password=device_pass
        )
    else:
        config = DeviceConfig.https(
            host=device_ip, username=device_user, password=device_pass, verify_ssl=False
        )
