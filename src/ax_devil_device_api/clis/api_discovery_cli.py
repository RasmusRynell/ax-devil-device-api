"""CLI for Device Configuration API discovery."""
...
def create_discovery_group():
    ...
    @click.pass_context
    def discovery(ctx):
        """Discover Device Configuration APIs and inspect their resources."""
    ...
    @click.option(
        "--docs-md", is_flag=True, help="Open markdown documentation in browser"
    )
    ...
    @click.option(
        "--docs-html", is_flag=True, help="Open HTML documentation in browser"
    )
        ...
        try:
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                if not api:
                    if version:
                        click.echo(
                            f"Error: API {api_name} version {version} not found",
                            err=True,
                        )
                    else:
...
def show_api_info(
    ...
    def handle_content(
        ...
        click.echo(click.style(f"\n{name}:", fg="yellow"))
        if (show or show_link) and not url:
            raise FeatureError(
                "missing_resource_url",
                f"No {name} URL available for {api.name} {api.version}",
            )
        full_url = _get_full_url(url) if url else None
