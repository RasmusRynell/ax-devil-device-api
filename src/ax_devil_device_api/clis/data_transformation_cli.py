...
def create_data_transformation_group():
    ...
    def list_topics(ctx):
        ...
        try:
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                for topic in result:
                    click.echo(f"  - {topic.get('topicName', topic.get('topic'))}")
                return 0
    ...
    @click.pass_context
    def create_transform(ctx, input_topic, output_topic, jq_expression):
        ...
        try:
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                click.echo(click.style("Transform created successfully!", fg="green"))
                click.echo("\nTransform details:")
    ...
    def remove_transform(ctx, output_topic, force):
        ...
        try:
            ...
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                click.echo(
                    click.style(
                        f"Transform '{output_topic}' removed successfully!", fg="green"
                    )
                )
                return 0
        except Exception as e:
