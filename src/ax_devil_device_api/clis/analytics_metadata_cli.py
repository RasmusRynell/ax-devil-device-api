            ...
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                if format == "json":
                    ...
                    for producer in producers:
                        ...
                        click.echo(
                            f"{producer.name:<30} {(producer.nice_name or ''):<40} {channels_str}"
                        )
    ...
    def enable_producer(ctx, producer_name, channel):
        ...
        try:
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                if not channel:
                    channels_to_enable = [
                        ch.channel for ch in target_producer.video_channels
                    ]
                else:
                ...
                invalid_channels = set(channels_to_enable) - advertised_channels
                if invalid_channels:
                    return handle_error(
                        ctx,
                        f"Producer '{producer_name}' does not advertise channel(s): "
                ...
                for ch in target_producer.video_channels:
                    enabled = ch.enabled or ch.channel in channels_to_enable
                    updated_channels.append(
                        VideoChannel(channel=ch.channel, enabled=enabled)
                    )
    ...
    def disable_producer(ctx, producer_name, channel):
        ...
        try:
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                invalid_channels = set(channels_to_disable) - advertised_channels
                if invalid_channels:
                    return handle_error(
                        ctx,
                        f"Producer '{producer_name}' does not advertise channel(s): "
