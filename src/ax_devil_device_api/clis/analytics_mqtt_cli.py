...
def create_analytics_group():
    ...
    @click.option("--qos", type=click.IntRange(0, 2), default=0, help="QoS level (0-2)")
        ...
        try:
            ...
            with create_client(**get_client_args(ctx.obj)) as client:
                client.analytics_mqtt.create_publisher(
                    id, source, topic, qos, retain, use_topic_prefix
                )
    ...
    def remove_publisher(ctx, id, force):
        ...
        try:
            ...
            with create_client(**get_client_args(ctx.obj)) as client:
                ...
                click.echo(
                    click.style(f"Publisher '{id}' removed successfully!", fg="green")
                )
                return 0
        except Exception as e:
            return handle_error(ctx, e)
